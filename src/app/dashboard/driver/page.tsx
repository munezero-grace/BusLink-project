...
import useDriverGuard from "./useDriverGuard";
        ...
        <div className="flex justify-between items-end mb-2 h-60 relative">
          ...
          {alertsByDay.map((dayData, index) => {
            ...
            const totalDayAlerts =
              dayData.seatBeltAlerts +
              dayData.overSpeedAlerts +
              dayData.harshBrakeAlerts +
              dayData.carIdlingAlerts;
            const barHeight =
              totalDayAlerts === 0 ? 0 : (totalDayAlerts / maxBarHeight) * 100;

            // Calculate individual segment heights
            const seatBeltHeight =
              (dayData.seatBeltAlerts / (totalDayAlerts || 1)) * barHeight;
            const overSpeedHeight =
              (dayData.overSpeedAlerts / (totalDayAlerts || 1)) * barHeight;
            const harshBrakeHeight =
              (dayData.harshBrakeAlerts / (totalDayAlerts || 1)) * barHeight;
            const carIdlingHeight =
              (dayData.carIdlingAlerts / (totalDayAlerts || 1)) * barHeight;

            return (
              <div
                key={index}
                className={`flex flex-col justify-end items-center mx-2 ${
                  index === selectedDayIndex ? "opacity-100" : "opacity-70"
                }`}
                style={{ width: "12%" }}
              >
        ...
        <div className="mt-8 grid grid-cols-2 gap-4">
          <div className="flex items-center">
            ...
            <span className="text-gray-700">
              Seat belt alerts -{selectedDay.seatBeltAlerts}
            </span>
          </div>
          <div className="flex items-center">
            <div className="w-6 h-3 bg-primary-dark mr-2"></div>
            <span className="text-gray-700">
              Harsh brake alerts -{selectedDay.harshBrakeAlerts}
            </span>
          </div>
          <div className="flex items-center">
            <div className="w-6 h-3 bg-green-500 mr-2"></div>
            <span className="text-gray-700">
              Over speed alerts -{selectedDay.overSpeedAlerts}
            </span>
          </div>
          <div className="flex items-center">
            <div className="w-6 h-3 bg-gray-700 mr-2"></div>
            <span className="text-gray-700">
              Car-idling alerts -{selectedDay.carIdlingAlerts}
            </span>
          </div>
        </div>
      </div>
